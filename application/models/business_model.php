    ...
    public function add_shop($data,$shop_category_ids)
    {
        ...
        if (!empty($_FILES['logo']['name'])) {
            ...
            $data['logo'] = "";
                ...
                //code for inserting shop signature
                $config['file_name'] = rand(10000, 10000000000);
                $config['upload_path'] = UPLOAD_PATH.'signature/';
                $config['allowed_types'] = 'jpg|jpeg|png|gif|pdf';
                $this->load->library('upload', $config);
                $this->upload->initialize($config);
                ...
                if (!empty($_FILES['signature']['name'])) {
                    //upload images
                    $_FILES['signatures']['name'] = $_FILES['signature']['name'];
                    $_FILES['signatures']['type'] = $_FILES['signature']['type'];
                    $_FILES['signatures']['tmp_name'] = $_FILES['signature']['tmp_name'];
                    $_FILES['signatures']['size'] = $_FILES['signature']['size'];
                    $_FILES['signatures']['error'] = $_FILES['signature']['error'];
        
                    if ($this->upload->do_upload('signatures')) {
                        $image_data2 = $this->upload->data();
                        $fileName2 = "signature/" . $image_data2['file_name'];
                    }
                    $data['signature'] = $fileName2;
                } else {
                    $data['signature'] = "";
                }
            ...
            if (!empty($fileName)) 
            {
                ...
                $data['logo'] = $fileName;
                        ...
                        if (!empty($_FILES['signature']['name'])) {
                            ...
                            {
                                if(DELETE_PATH.is_file($data2->signature))
                                {
                                    unlink(DELETE_PATH.$data2->signature);
                                }
                            }
                            $data['signature'] = $fileName2;
                        } 
