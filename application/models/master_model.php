        ...
        public function add_duplicate_product($data,$duplicate_id)
    {
        ...
        if (!empty($imageCount)) {
            ...
            for ($i = 0; $i < $imageCount; $i++)  {
                ...
                $config[' _name'] = date('Ymd') . rand(1000, 1000000);
          ...
          $config['file_name'] = rand(10000, 10000000000);    
        $config['upload_path'] = UPLOAD_PATH.'size_chart/';
        ...
        if (!empty($_FILES['chart']['name'])) {
            ...
            //upload images
            $_FILES['charts']['name'] = $_FILES['chart']['name'];
            $_FILES['charts']['type'] = $_FILES['chart']['type'];
            $_FILES['charts']['tmp_name'] = $_FILES['chart']['tmp_name'];
            $_FILES['charts']['size'] = $_FILES['chart']['size'];
            $_FILES['charts']['error'] = $_FILES['chart']['error'];

            if ($this->upload->do_upload('charts')) {
                $image_data = $this->upload->data();
             
                $fileName = "size_chart/" . $image_data['file_name'];
            }
            $data['size_chart'] = $fileName;
        } else {
            $rschart = $this->master_model->getRow('products_subcategory',['id'=>$duplicate_id]);
            $data['size_chart'] = @$rschart->size_chart;
        }
        ...
        if (!empty($_FILES['chart_inch']['name'])) {

            //upload images
            $_FILES['chart_inchs']['name'] = $_FILES['chart_inch']['name'];
            $_FILES['chart_inchs']['type'] = $_FILES['chart_inch']['type'];
            $_FILES['chart_inchs']['tmp_name'] = $_FILES['chart_inch']['tmp_name'];
            $_FILES['chart_inchs']['size'] = $_FILES['chart_inch']['size'];
            $_FILES['chart_inchs']['error'] = $_FILES['chart_inch']['error'];

            if ($this->upload->do_upload('chart_inchs')) {
                $image_data = $this->upload->data();
             
                $fileName = "size_chart/" . $image_data['file_name'];
            }
            $data['size_chart_inch'] = $fileName;
        } else {
            $rschart = $this->master_model->getRow('products_subcategory',['id'=>$duplicate_id]);
            $data['size_chart_inch'] = @$rschart->size_chart_inch;
        }
        if (!empty($images))
        {     
...
//                //end sitemap code
    ...
    public function products($parent_id,$pro_id,$cat_id,$child_cat_id,$search,$limit=null,$start=null)
    {
        if ($limit!=null) {
            $this->db->limit($limit, $start);
        }
 ...
 public function get_property_val_new($id)
{
    ...
    public function get_property_val_by_id($id)
    {
        ...
        public function fetch_products($id)
        {
            ...
            ->where(['t3.cat_id' => $id,'t2.is_cover' =>'1','t1.is_deleted' =>'NOT_DELETED'])
            ->get();
            return $query->result();
        }
        ...
        public function delete_category($id)
        {
            ...
            if($r->level=='1')
            {
                ...
                foreach($res as $row)
                {
                    ...
                    $is_deleted = array('is_deleted' => 'DELETED');
                    $this->db->where('id', $row->id)->update('products_category', $is_deleted);
                }
                foreach($ids as $i)
                {
                    $this->db->select('*');
                    $this->db->from('products_category');
                    $this->db->where('is_parent',$i);
                    $is_deleted = array('is_deleted' => 'DELETED');
                    $this->db->update('products_category', $is_deleted);
                }
                $is_deleted = array('is_deleted' => 'DELETED');
                return $this->db->where('id', $id)->update('products_category', $is_deleted);
            }
            else if($r->level=='2')
            {
                $this->db->select('*');
        ...
        if (!empty($_FILES['chart']['name'])) {
            ...
            $rschart = $this->master_model->getRow('products_subcategory',['id'=>$id]);
            $data['size_chart'] = @$rschart->size_chart;
        }
...
public function getProductDetails($shop_id,$search)
{
    ...
    $this->db->select("a.*,b.thumbnail,c.qty,c.mrp,c.purchase_rate,c.selling_rate,c.id as inventory_id,c.shop_id,d.name as flavour")
    ->from('products_subcategory a')
    ->join('products_photo b','b.item_id=a.id','left')
    ->join('shops_inventory c','c.product_id=a.id','left')
    ->join('flavour_master d','d.id=a.flavour_id','left')
...
public function warehouse_master($limit=null,$start=null)
{
    if ($limit!=null) {
        $this->db->limit($limit, $start);
    }
    ...
    ->select('t1.*,t2.shop_name,t3.title')
    ->join('shops t2', 't2.id = t1.shop_id','left') 
    ->join('business t3', 't3.id = t2.business_id','left') 
    ->where(['t1.is_deleted'=>'NOT_DELETED'])
    ->order_by('t1.added','desc');                   
...
{
    $cond = array('shop_id' => $shop_id, 'is_deleted' => 'NOT_DELETED', 'isActive' => 1, 'user_type' => 'customer');
    return $this->db->order_by('fname','asc')->get_where('customers',$cond)->result();
}
public function fill_products()
{
    $id = $_POST['sub_cat_id'];
    $this->db->select("mtb.*, CONCAT(
                        IF(psc.name IS NULL , '' , ' / '),
                        IF(psc.name IS NULL , '', psc.name)
                        ) as Category ");
    $this->db->from('products_subcategory mtb');
    $this->db->join('cat_pro_maps cp', 'cp.pro_id = mtb.id','left');
    $this->db->join('products_category psc', 'cp.cat_id = psc.id','left');
    $this->db->where(['cp.cat_id' => $id]);
    if (@$_POST['vendor_id']) {
        $this->db->join('shops_inventory si', 'si.product_id = mtb.id','left');
        $this->db->where('si.vendor_id',$_POST['vendor_id']);
    }
    if (@$_POST['brand_id']) {
        $this->db->where('mtb.brand_id',$_POST['brand_id']);
    }
    $this->db->where('mtb.is_deleted','NOT_DELETED');
    $this->db->where('mtb.active',1);
    $this->db->order_by('mtb.name','ASC');  
    $this->db->group_by('mtb.id');  

    return $this->db->get()->result();
    
}

public function fill_products_new()
{
    $this->db->select("mtb.*, CONCAT(pc.name, ' ' ,
                        IF(psc.name IS NULL , '' , ' / '),
                        IF(psc.name IS NULL , '', psc.name)
                        ) as Category ");
    $this->db->from('products_subcategory mtb');
...
public function fill_products_invoice()
{
    $this->db->select("t2.*,t3.*");
    $this->db->from('pos_orders t1');
    $this->db->join('pos_order_items t2', 't2.order_id = t1.id','left');
    $this->db->join('products_subcategory t3', 't3.id = t2.product_id','left');

    if (@$_POST['invoice_no']) {
        $this->db->where('t1.orderid',$_POST['invoice_no']);
    }
    $this->db->where('t3.is_deleted','NOT_DELETED');
...
public function fetchCustomer()
{
    ...
    $this->db->from('pos_orders t1');
    $this->db->join('customers t2','t1.user_id=t2.id','left');
    if (@$_POST['invoice_no']) {
        $this->db->where('t1.orderid',$_POST['invoice_no']);
    } 
    if (@$_POST['invoice_no']) {
...
{
    $this->db->select("t1.*");
    $this->db->from('pos_order_items t1');
    if (@$_POST['pro_id']) {
        $this->db->where('t1.product_id',$_POST['pro_id']);
    } 
    if (@$_POST['pro_id']) {
    return $this->db->get()->row();
    }else{
...
public function fill_products_new_aging()
{
    $this->db->select("mtb.*");
    $this->db->from('products_subcategory mtb');
    if (@$_POST['vendor_id']) {
...
public function productBarcode($limit=null,$start=null)
{
    if ($limit!=null) {
        $this->db->limit($limit, $start);
    }
    ...
    if (@$_POST['search']) {
        $this->db->group_start();
        $this->db->like('t1.name', $_POST['search']);
        $this->db->or_like('t1.search_keywords', $_POST['search']);
        $this->db->or_like('t1.product_code', $_POST['search']);
        $this->db->group_end();
    }
    if($limit!=null)
        return $this->db->get()->result();
...
public function getBarcodeProduct($pid)
{
    $this->db
    ->select('t1.*,t2.mrp,t3.shop_name,t3.email as shop_email,t4.name  as state_name,t5.name  as city_name,t3.address,t3.pin_code')
    ->from('products_subcategory t1')
    ->join('shops_inventory t2','t1.id=t2.product_id','left')
    ->join('shops t3','t2.shop_id=t3.id','left')
    ->join('states t4','t4.id=t3.state','left')
    ->join('cities t5','t5.id=t3.city','left')
