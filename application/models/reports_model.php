...
class reports_model extends CI_Model
{
    ...
    public function get_tax_report_input($shop_id,$from_date,$to_date,$status)
    {
       
        $this->db
        ->select('t1.*,t3.name as cust_state,t5.name as shops_state')
        ->from('purchase t1')
        ->join('customers t2', 't2.id = t1.supplier_id','left')   
        ->join('states t3', 't3.id = t2.state','left')   
        ->join('shops t4', 't4.id = t1.shop_id','left')  
        ->join('states t5', 't5.id = t4.state','left')     
		->where('t1.shop_id',$shop_id)
        ->where('t1.status', '3')
		->where(['t1.purchase_order_date >='=>$from_date , 't1.purchase_order_date <='=>$to_date]);
        if ($status!='null') {
			$this->db->where('t1.status' , $status);
		}
    ...
    public function get_tax_report_output($shop_id, $from_date, $to_date, $status)
    {
      
    
        // Query for pos_orders
        $this->db
            ->select('t1.id, t1.total_value, t1.round_off, t1.tax, t3.name as cust_state, t5.name as shops_state')
            ->from('pos_orders t1')
...
public function export_purchase_report($shop_id,$from_date,$to_date,$vendor,$search,$brand_id,$parent_id,$parent_cat_id,$child_cat_id,$limit=null,$start=null)
    ...
    public function get_sales_report($pro_id,$shop_id,$from_date,$to_date,$payment_mode,$search,$status_id,$brand_id,$parent_cat_id,$child_cat_id,$product_id,$subscription,$plan_type_id,$limit=null,$start=null)
    {
        ...
        $this->db
        ->select('t1.id as order_id,t1.*,t2.*,t3.*,t4.product_code,t4.name as product_name,t4.sku,t4.parent_cat_id,t4.sub_cat_id,t4.id as prod_id,t1.added as order_date,t5.name as brand_name,t7.thumbnail')
        ->from('orders t1')       
    ...
    public function export_sales_report($shop_id,$from_date,$to_date,$payment_mode,$status_id,$search,$brand_id,$parent_id,$parent_cat_id,$child_cat_id,$product_id,$subscription,$plan_type_id)
    {
        ...
        $this->db
        ->select('t1.id as order_id,t1.*,t2.*,t3.*,t4.product_code,t4.name as product_name,t4.sku,t4.parent_cat_id,t4.sub_cat_id,t1.added as order_date,t5.name as brand_name,t4.id as prod_id')
        ->from('orders t1')       
        ->join('order_items t2', 't2.order_id = t1.id') 
    ...
    public function get_pos_sales_report($from_date,$to_date,$customer_id,$parent_id,$parent_cat_id,$sub_cat_id,$product_id,$brand_id,$status_id,$tb_search,$pro_id,$shop_id, $limit = null, $start = null)
    {
        ...
        if (!empty($from_date) && $from_date!='null' && !empty($to_date) && $to_date!='null') {
            $this->db->where(['DATE(t1.datetime) >=' => $from_date, 'DATE(t1.datetime) <=' => $to_date]);
        }
    
        if (!empty($tb_search) && $tb_search !='null') {
            $search = $tb_search;
            $this->db->group_start();
        ...
        if (!empty($status_id) && $status_id !='null') {
            $this->db->where('t1.status', $status_id);
        }
        if (!empty($customer_id) && $customer_id !='null') {
            $this->db->where('t1.user_id', $customer_id);
        }
    ...
    public function calculate_pos_sales_report($from_date,$to_date,$customer_id,$parent_id,$parent_cat_id,$sub_cat_id,$product_id,$brand_id,$status_id,$tb_search,$pro_id,$shop_id)
    {
