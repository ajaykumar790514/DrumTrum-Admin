...
class shop_inventory_logs_model extends CI_Model
{
	function index(){
		echo 'This is model index function';
	}
	function __construct(){
		$this->tbl1 = 'shop_inventory_logs';
		$this->load->database();
	}
	...
	function insertRow1($data = array()){
		$result = $this->db->insert_id($this->tbl1,$data);
		return $result;
	}
	...
	function getMaxRow($shop_inventory_id){
		$this->db->select("*");
		$this->db->from($this->tbl1);
		// $this->db->where('action','LATEST_UPDATE');
		$this->db->where('shops_inventory_id',$shop_inventory_id);
