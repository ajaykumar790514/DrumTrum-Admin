...
class Welcome extends CI_Controller {
	...
	public function dashboard(){
		...
		$data['shop_details']=$shop_details = $this->shops_model->get_shop_data($shop_id);
		...
		if(!empty($user)){
			$template_data = array(
									...
									'main_body_data'=>$this->load->view('template/sample_dashboard',$data,TRUE),
   ...
   public function dashboard_total()
   {
	...
	if ($this->input->is_ajax_request()) {
		...
		list($startDate, $endDate) = explode(' - ', $typeDate);
		$startDate = date('Y-m-d', strtotime(str_replace('/', '-', $startDate)));
		$endDate = date('Y-m-d', strtotime(str_replace('/', '-', $endDate)));
		$data = $this->shops_model->fetch_data($startDate, $endDate);
	   if(!empty($data))
	   {
		$return['success']=true;
		$return['data'] = $data;
