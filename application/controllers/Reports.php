...
class Reports extends CI_Controller {
        ...
        switch ($action) {
            case null:
                ...
                case 'tb':
                    ...
                    { 
                    $shop_id     = $user->id;
                    $data['tax_report_input']           = $this->reports_model->get_tax_report_input($shop_id,$from_date,$to_date,$status_id);
                    $tax_report_output           = $this->reports_model->get_tax_report_output($shop_id,$from_date,$to_date,$status_id);
                    
                    $data['Outputorders'] = $tax_report_output['orders'];
                    $data['OutputposOrders'] = $tax_report_output['posOrders'];
                    }
                    $data['shops'] = $this->shops_model->get_shop_detail($user->id);
                    $data['order_status'] = $this->master_model->get_data1('order_status_master','active','1');
                    $data['empty'] = "";
                ...
                if (@$_POST['parent_cat_id'] || @$_POST['child_cat_id']) {
                    ...
                    case 'export_to_excel':
                        ...
                        foreach($result as $value){
                            ...
                            $sheet->setCellValue('E'.$count, $value->fname.' '.$value->lname);
    ...
    public function fetch_category()
    {
     ...
     {
         $data = $this->master_model->fill_products_invoice();
     ...
     {
        $data = $this->master_model->fetchCustomer();
        $customer_name = $data->fname. $data->lname ;
        $response = array(
            'customer_id' => isset($data->id) ? $data->user_id : '',
            'customer_name' => $customer_name  ? $customer_name: '',
            'order_date' => isset($data->datetime) ? date("d-m-Y",strtotime($data->datetime)) : '',
            'reference_no' =>isset($data->reference_no_or_remark) ? $data->reference_no_or_remark : '',
        );
     ...
     public function fetchProductDetails()
     {
        ...
        $data = $this->master_model->fetchProductDetails();
        $response = array(
            'status' =>true,
            'data' => $data,
        );
        ...
        $data['user']  = $user         = $this->checkShopLogin();
         ...
         switch ($action) {
             ...
             case 'tb':
                ...
                $tb_search='null';
                $pro_id = array();
                $data['from_date'] =$from_date= !empty($p['from_date']) ? $p['from_date'] : 'null';  
                $data['to_date'] =$to_date= !empty($p['to_date']) ? $p['to_date'] : 'null';  
                $data['customer_id'] =$customer_id= !empty($p['customer_id']) ? $p['customer_id'] : 'null';  
                $data['parent_id'] =$parent_id= !empty($p['parent_id']) ? $p['parent_id'] : 'null';  
                $data['parent_cat_id'] =$parent_cat_id= !empty($p['parent_cat_id']) ? $p['parent_cat_id'] : 'null';  
                $data['sub_cat_id'] = $sub_cat_id=!empty($p['sub_cat_id']) ? $p['sub_cat_id'] : 'null';  
                $data['product_id'] =$product_id= !empty($p['product_id']) ? $p['product_id'] : 'null';  
                $data['brand_id'] =$brand_id= !empty($p['brand_id']) ? $p['brand_id'] : 'null';  
                $data['status_id'] = $status_id=!empty($p['status_id']) ? $p['status_id'] : 'null';  
                $data['tb_search'] = $tb_search=!empty($p['search']) ? $p['search'] : 'null'; 
                if (@$_POST['parent_cat_id']) {
                    $data['parent_cat_id'] = $_POST['parent_cat_id'];
                    $parent_cat_id=$_POST['parent_cat_id'];
                    $data['parent_id'] = $_POST['parent_id'];
                ...
                if(!empty($p1))
                {
                    $data['from_date'] =$from_date= $p1;  
                }
                if(!empty($p2))
                {
                    $data['to_date'] =$to_date= $p2;  
                }
                if(!empty($p3))
                {
                    $data['customer_id'] =$customer_id= $p3;  
                }
                if(!empty($p7))
                {
                    $data['product_id'] =$product_id= $p7;  
                }
                if(!empty($p8))
                {
                    $data['brand_id'] =$brand_id= $p8;  
                }
                if(!empty($p9))
                {
                    $data['status_id'] =$status_id= $p9;  
                }
                if(!empty($p10))
                {
                    $data['tb_search'] =$tb_search= $p10;  
                }
                if (!empty($p5)) {
                    $data['parent_cat_id'] = $p5;
                    $parent_cat_id=$p5;
                    $data['parent_id'] = $p4;
                    $parent_id=$p4;
                    $data['sub_cat'] = $this->db->get_where('products_category',['is_parent' => $parent_id , 'is_deleted' => 'NOT_DELETED'])->result();
                ...
                if (!empty($p6)) {
                    $data['child_cat_id'] = $p6;
                    $sub_cat_id=$p6;
                    $data['child_cat'] = $this->db->get_where('products_category',['is_parent' => $parent_cat_id , 'is_deleted' => 'NOT_DELETED'])->result();

                    $pro_id = array();
                    $get_proid = $this->db->get_where('cat_pro_maps',['cat_id' => $p6])->result();
                    foreach($get_proid as $row){
                        $pro_id[] = $row->pro_id;
                    }
                }
                $data['per_page']=$p11;
                     ...
                     $shop_id    = $user->id;
                     $config["base_url"]         = base_url() . "pos-sale-register/tb/".$from_date."/".$to_date."/".$customer_id."/".$parent_id."/".$parent_cat_id."/".$sub_cat_id."/".$product_id."/".$brand_id."/".$status_id."/".$tb_search;
                     $config["total_rows"]       = $this->reports_model->get_pos_sales_report($from_date,$to_date,$customer_id,$parent_id,$parent_cat_id,$sub_cat_id,$product_id,$brand_id,$status_id,$tb_search,$pro_id,$shop_id);
                     ...
                     $data['page']               = $page = ($p11 != null) ? $p11 : 0;
                     $data['per_page']           = $config["per_page"];
 
                     $data['sales_report']           = $this->reports_model->get_pos_sales_report($from_date,$to_date,$customer_id,$parent_id,$parent_cat_id,$sub_cat_id,$product_id,$brand_id,$status_id,$tb_search,$pro_id,$shop_id,$config["per_page"], $page);
                    //  echo $this->db->last_query();die();
                     $data['categories']     = $this->reports_model->view_data('products_category');
                     $data['sales_result'] = $this->reports_model->calculate_pos_sales_report($from_date,$to_date,$customer_id,$parent_id,$parent_cat_id,$sub_cat_id,$product_id,$brand_id,$status_id,$tb_search,$pro_id,$shop_id);
                     $data['cat_pro_map']         = $this->master_model->get_cat_pro_map_for_product_list();
                     $data['parent_cat'] = $this->master_model->get_data('products_category', 'is_parent', '0');
                     $data['brands'] = $this->master_model->get_brands($shop_id);
                     $data['customers'] = $this->master_model->get_customers($shop_id);
