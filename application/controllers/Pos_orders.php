...
class Pos_orders extends CI_Controller {
    ...
    public function getOrders() {
        ...
        if (isset($_SESSION['order_table_filters']['orderid']) && $_SESSION['order_table_filters']['orderid'] !== '') {
            $search_term = $_SESSION['order_table_filters']['orderid']; // or whatever your search input is called
        }
        ...
        $getData = $this->pos_orders_model->getOrdersData($arrayQuery, $mobile, $payment_mode, null, $search_term);

        $array =array();
        $item_count=0;
        if($getData!==FALSE){
            $sale_return= $this->db->get_where('tb_admin_menu',['url'=>'sale_return'])->row();
            $i=0;
            foreach($getData as $data){
                ...
                $array[$i] = array(
                                    ...
                                    'due_date' => $due_date,
                                    'order_type'=>($data['order_type']=='Walk In') ? 'Walk In' : 'Online',
                                    'payment_mode'=>$data['pos_payment_mode_name'],
                                    'payment_status'=>$data['payment_mode'],
                                    'due_value' => ($data['payment_mode'] == 4) ? '₹ '.$data['total_value'] : '₹ 0.00',
                                    'reference_no_or_remark'=> $data['reference_no_or_remark'] ,
                );
                if ($data['status']==='1') {
                    $array[$i]['print_bill'] = array('orderid'=>'0','order'=>'0','row_id'=>'0');
                    $array[$i]['action'] = array('orderid'=>'0','sale_return'=>$sale_return->id,'row_id'=>'0');
                    
                }else{
                    $array[$i]['print_bill'] = array('orderid'=>$data['orderid'],'order'=>$data['id'],'row_id'=>$data['id']);    
                    $array[$i]['action'] = array('orderid'=>$data['orderid'],'sale_return'=>$sale_return->id,'row_id'=>$data['id']);  
                }
    ...
    public function orderDetails($order_id){
		...
		// if($this->session->has_userdata('logged_in') && $this->session->logged_in === TRUE){
            ...
            if(@$viewData['orderData'][0]['address_id']!==''){
                $viewData['customerData'] =  $customerData = $this->customers_model->getRows(array('conditions'=>array('id'=>@$viewData['orderData'][0]['user_id'])));
    ...
    public function orderPrintBill($orderId) {
        ...
        if($data['shop']->bill_size=='A4'){
            $this->load->view('template/pos_order_bill_print_layout',$data);
        }else{
            $data['invoice'] = $this->pos_orders_model->invoice_details_new($orderId);
            $this->load->view('template/pos_order_bill_print_layout_new',$data);
        }
        
    }
    public function orderPrintBillHold($orderId) {
        $invoice = $this->pos_orders_model->invoice_details($orderId);
        $data['order']  = $invoice['order'];
        $data['vendor'] = $invoice['vendor'];
        $data['shop']   = $invoice['shop'];
     ...
     public function proforma_invoice() {
        ...
        if($data['shop']->bill_size=='A4'){
            ...
            $this->load->view('template/pos_po_print_layout_new',$data);
        }
