...
class Stocks extends CI_Controller {
    ...
    public function getStockData(){
        ...
        $array_cond['cat_id'] = $_GET['parent_cat_id'];
        // $array_cond['active'] = '1';
    ...
    {
        $id = $_POST['parent_cat_id'];
        $getData = $this->products_subcategory_model->getRows($id);
        ...
        foreach ($getData as $data) {
            $temp_array = array(
                'id' => $data['id'],
                'name' => str_pad($data['product_code'], 6, '0', STR_PAD_LEFT) . ' - ' . $data['name'].' - '.$data['unit_value'] .' '.$data['unit_type'],
                'gst' => $data['tax_value']
            );
        ...
        switch ($action) {
            ...
            case 'tb':
                ...
                case 'new-purchase':
                    ...
                    $data['suppliers']      = $this->master_model->getData('customers',['isActive'=>'1','user_type'=>'supplier','is_deleted'=>'NOT_DELETED']);
                    $data['shop_details'] = $this->shops_model->get_shop_data($shop_id);
                ...
                case 'search_supplier':
                    ...
                    foreach ($suppliers as $supplier) {
                        $results1[] = [
                            'id' => $supplier->id,
                            'text' => $supplier->fname.' '.$supplier->lname
                        ];
                    }
                    $results = array(
                        'items' => $results1,
               ...
               case 'supplier-modal':
                ...
                case 'supplier-save':
                    ...
                    if ($this->input->server('REQUEST_METHOD')=='POST') { 
                            ...
                            if ($insert_id = $this->shops_vendor_model->add_data('customers',$data)) {
                                $opening = array(
                                    'user_id' => $insert_id,
               ...
               case 'add-purchase':
                ...
                $SupData =$this->shops_vendor_model->getRow('customers',['id'=>$supplier_id,'user_type'=>'supplier']);
                $purchase = array(
                ...
                'remark'=>$notes,
                'status'=>'3',
                'billing_address'=>@$SupData->address,
                'billing_state'=>@$SupData->state,
                'billing_city'=>@$SupData->city,
                'billing_pincode'=>@$SupData->pincode,
                );
                ...
                foreach ($itemCodes as $index => $itemCode) {
                      ...
                      $html="";
                      $res=$this->shops_vendor_model->product_props($productVariants[$index]);
                       {
                          if(count($res)>0)
                          {
                            foreach($res as $row)
