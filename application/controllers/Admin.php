    ...
    public function update_admin_password()
	{
        ...
        if($result)
        {
            if ($this->admin_model->edit_data('admin','1',$data)) {
                $this->session->set_tempdata('success', 'Password Changed Successfully..', 1);
                redirect($this->agent->referrer());
            } else {
                $this->session->set_tempdata('error', 'Password Not Changed!', 1);
                redirect($this->agent->referrer());
            }
        }
        else
        {
            $this->session->set_tempdata('error', 'Old Password Does not match!', 1);
			redirect($this->agent->referrer());
        }
	}
