...
class Sale_return extends CI_Controller
{
    ...
    public function store()
    {
        ...
        if (@$_POST['customer_id'] && @$_POST['product_id'] && @$_POST['return_qty'] && @$_POST['return_rate'] && @$_POST['return_total'] && @$_POST['stock_id']) {
            ...
            $pro_max_qty = $post['pro_max_qty'];
            $invoice_no = $post['invoice_no'];

            // Get existing quantity for the product
            $this->db->select_sum('qty');
            $this->db->where('product_id', $post['product_id']);
            $this->db->where('original_invc_no', $post['invoice_no']);
            $this->db->where('inventory_id', $post['stock_id']);
            $total_returned_qty = $this->db->get('sales_return')->row()->qty;

            if (($qty) > $pro_max_qty) {
                $response['res'] = 'error';
                $response['msg'] = 'Return quantity exceeds the orders maximum quantity!';
                echo json_encode($response);
                return; 
            }

            if (($total_returned_qty + $qty) > $pro_max_qty) {
                $response['res'] = 'error';
                $response['msg'] = 'Return quantity exceeds the allowed maximum quantity!';
                echo json_encode($response);
                return; 
            }
            ...
            $insertArray = array(
                ...
                'original_invc_no' => $post['invoice_no'],
