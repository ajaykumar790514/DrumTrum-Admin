...
<div class="row">
    ...
    function save_order(clickFrom) {
        ...
        var RoundOff            = $('#sale_totals').find('.RoundOff').text();
                            ...
                            success: function(data) {
                                ...
                                if (data.res=='success') {
                                   Swal.fire({
                                      ...
                                      position: 'center',
                                      footer: ' <a href="'+data.new_order+'" class="btn btn-sm btn-primary mr-2">New Order</a><a href="'+data.invoice_url+'" class="btn btn-sm btn-info mr-2">Invoice</a>'
                                    }) 
                                }
                                else{
                                   Swal.fire({
            ...
            <div class="card-body pb-0">
                ...
                <div class="row mb-4">
                    <div class="col-12 text-center">
                        <button class="btn btn-sm btn-success save_order">Save Order</button>
                    </div>
                </div>
                ...
                <div class="form-group mb-0" id="select_customer">
                    ...
                    <p></p>
                    <div class="center">
                        <button class="btn btn-sm btn-primary" href="javascript:void(0)" data-toggle="modal" data-target="#showModal" data-whatever="Add Customer" data-url="<?=$new_customer?>" ><span class="fa fa-user">&nbsp;</span>New Customer</button>&nbsp;&nbsp;
                        <button class="btn btn-sm btn-secondary bg-secondary text-white" href="javascript:void(0)" data-toggle="modal" data-target="#" data-whatever=""><span class="fa fa-share">&nbsp;</span>Shortcuts </button>
                    </div>
                </div>
            </div>
            ...
            <div id="tb">
                ...
                <div class="row pb-2">
                    ...
                    <div class="col-12 text-center mt-3">
                        <button class="btn btn-sm btn-success proforma_invoice">Proforma Invoice</button>
                    </div>
                </div>
            </div>
            <style type="text/css">
...
let vGST = (gnumber)=>{
    ...
    $("#item").autocomplete({
        ...
        select: function(event, ui) {
            ...
            if (inventoryId != ui.item.inventory_id) {
                ...
                var TaxValue = getV[1];
                ProductTable += "<tr class='jsgrid-filter-row' data-product_code='"+ui.item.product_code+"'><td class='jsgrid-cell jsgrid-align-center'><a href='javascript:void(0)' id='DeleteButton' class='btn  btn-sm' ><i class='fa fa-trash' style='color:red'></i></a></td>";
            ...
            if ($('#item-table >tbody >tr').length == 2 && inventoryId != ui.item.inventory_id) {
                ...
                var finalTotal = (Number(AmountwithoutTax) + Number(TaxValue)).toFixed(2);
                ...
                TotalSaleTb = '<tbody id="sale-record1">\
                            ...
                            <tr>\
                              ...
                              <tr>\
                                <th style="width: 55%;">Round Off</th>\
                                <td style="width: 45%; text-align: right;" class="RoundOff">' + roundoff + '</td>\
                            </tr>\
                            <tr>\
                                <th style="width: 55%; font-size: 150%">Total</th>\
                                <td style="width: 45%; font-size: 150%; text-align: right;" class="TotalValue">' + newAmount + '</td>\
                            </tr>\
                        </tbody>';
    ...
    const originalAmount = parseFloat(amount).toFixed(2); // Rounded to 2 decimal places
    const roundedAmount = (Math.round(amount * 100) / 100).toFixed(2); // Rounded to 2 decimal places
    const newAmount = (Math.ceil(amount)).toFixed(2); // Rounded up to the next whole number

    return {
        originalAmount: originalAmount,
        roundedAmount: roundedAmount,
        newAmount: newAmount,
        roundoff: (newAmount-roundedAmount).toFixed(2)
    };
}
    ...
    function GetSaleRecord(SellingRate, TaxValue, Stock) {
        ...
        var TaxValue = (SellingRate*TaxValue)/100;
        ...
        if (Number(Thisquantity) != '' && Number(Stock) >= Number(Thisquantity)) {
            $('#sale_totals > tbody  > tr').each(function(i, j) {
                ...
                if ($(this).find(".RoundOff").text() != '') {
                    $(this).find(".RoundOff").text((Number(roundoff).toFixed(2)));
                }
                if ($(this).find(".TotalValue").text() != '') {
                    $(this).find(".TotalValue").text((Number(newAmount).toFixed(2)));
                }
