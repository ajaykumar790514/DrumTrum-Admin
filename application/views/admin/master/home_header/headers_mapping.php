                                    ...
                                    <div class="col-12">
                                        <div id="grid_table">
                                            <table class="jsgrid-table">
                                                ...
                                                $rs = $this->master_model->get_property_val_new($value->prod_id); ?>
                                                <tr class="jsgrid-filter-row">
                                                    ...
                                                    <td class="jsgrid-cell jsgrid-align-center"><?php echo $value->prod_name;?><br><?php if(!empty($rs)){?><strong class="text-danger"> [ <?php foreach($rs as $r){echo $r->value.",";};?> ] </strong><?php }?></td>
                                                    <td class="jsgrid-cell jsgrid-align-center">
                                                    <img src="<?php echo IMGS_URL.$value->thumbnail; ?>" alt="image" height="100">
                                                    </td>
...
<script type="text/javascript">
   ...
   {
    $.ajax({
        url: "<?php echo base_url('master-data/add_mapping'); ?>",
        method: "POST",
        data: {
            headerid:headerid
        },
    ...
    // $("#mapping").load("<?php echo base_url('master-data/add_mapping'); ?>"+headerid)
   }
</script>
<script type="text/javascript">
...
{
$.ajax({
    url: "<?php echo base_url('master-data/fetch_sub_category'); ?>",
    method: "POST",
    data: {
        parent_id:parent_id,
    },
...
<script type="text/javascript">
   function fetch_products(parent_cat_id)
   {
    ...
    $.ajax({
        url: "<?php echo base_url('master-data/fetch_products'); ?>",
        method: "POST",
        data: {
...
<script type="text/javascript">
   function map_product(pid)
   {
    var headerid = $('#headerid').val();
    $.ajax({
        url: "<?php echo base_url('master-data/map_product'); ?>",
        method: "POST",
        data: {
            pid:pid,
            headerid:headerid
...
<script type="text/javascript">
   function remove_map_product(pid)
   {
    var headerid = $('#headerid').val();
    $.ajax({
        url: "<?php echo base_url('master-data/remove_map_product'); ?>",
        method: "POST",
        data: {
            pid:pid,
            headerid:headerid
