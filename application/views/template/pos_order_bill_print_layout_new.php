  ...
  <!-- onload="window.print()" -->
    <section class="main-page">
       ...
       <div class="customer-div row">
        ...
        <?php
          ...
          $total_=0;
            ...
            foreach($invoice as $key=> $row):
                ...
                {
                  $amount2 = ($row['selling_rate']-$row['offer_applied']);
                  $discount = $row['selling_rate']-$amount2;
                  $Tdiscount +=$row['selling_rate']-$amount2;
                ...
                {
                  $amount = ($amount-$row['offer_applied2']);
        ...
        <?php
        $i=1;$payments = $this->pos_orders_model->OrderPayments($invoice[0]['oid']);
        foreach ($payments as $payment) {
          if($i==1){
          echo '<div class="col-12" style="margin-top:-10px">';
          }else{
            echo '<div class="col-12" style="margin-top:-2px">'; 
          }
          if($payment['mode_name']=='Paylater')
          {
            echo '<span style="float:left">Due Amount</span>';
          }else{
            echo '<span style="float:left">By ' . $payment['mode_name'] . '</span>';
          }
          
          echo '<span style="float:right;position: relative;right: 15px;">' . $shop->currency . number_format((float)($payment['amount']), 2, '.', '') . '</span>';
          echo '</div>';
       $i++; }
        ?>
